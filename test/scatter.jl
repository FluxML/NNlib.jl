    ...
    @testset "$T" begin
        ...
        @testset "scatter_add!" begin
            ...
            for xs in [xs_int, xs_tup]
                @test scatter!(+, T.(copy(ys)), T.(us), xs) == T.(ys_)
                @test scatter!(+, T.(copy(ys)), us, xs) == PT.(ys_)
                @test scatter!(+, copy(ys), T.(us), xs) == PT.(ys_)
            end
        end
        ...
        @testset "scatter_max!" begin
            ...
            for xs in [xs_int, xs_tup]
                ...
                @test scatter!(max, T.(copy(ys)), us, xs) == PT.(ys_)
                @test scatter!(max, copy(ys), T.(us), xs) == PT.(ys_)
            end
        end
        ...
        @testset "scatter_min!" begin
            ...
            for xs in [xs_int, xs_tup]
                ...
                @test scatter!(min, T.(copy(ys)), us, xs) == PT.(ys_)
                @test scatter!(min, copy(ys), T.(us), xs) == PT.(ys_)
            end
        end
...
for T = [Float16, Float32, Float64, BigFloat, Rational]
    @testset "$T" begin
        ...
        @testset "scatter_div!" begin
            ...
            for xs in [xs_int, xs_tup]
                @test scatter!(/, T.(copy(ys)), T.(us_div), xs) == T.(ys_)
                @test scatter!(/, T.(copy(ys)), us_div, xs) == PT.(ys_)
                @test scatter!(/, copy(ys), T.(us_div), xs) == PT.(ys_)
            end
        end
        ...
        @testset "scatter_mean!" begin
            ...
            for xs in [xs_int, xs_tup]
                ...
                @test scatter!(mean, T.(copy(ys)), us, xs) == PT.(ys_)
                @test scatter!(mean, copy(ys), T.(us), xs) == PT.(ys_)
            end
        end
    end
end
