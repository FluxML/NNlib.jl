...
function.
"""
@inline function σ(x::RealOrFloatType)
    t = exp(-abs(x))
...
"""
@inline hardσ(x::RealOrFloatType, a=0.2) = oftype(x/1, max(zero(x/1), min(one(x/1), oftype(x/1,a) * x + oftype(x/1,0.5))))
...
"""
@inline logσ(x::RealOrFloatType) = -softplus(-x)
...
"""
@inline hardtanh(x::RealOrFloatType) = max(-one(x), min( one(x), x))
...
activation function.
"""
@inline relu(x::RealOrFloatType) = max(zero(x), x)
...
"""
@inline leakyrelu(x::RealOrFloatType, a = oftype(x / 1, 0.01)) = max(a * x, x / one(x))
...
"""
@inline relu6(x::RealOrFloatType) = min(relu(x), oftype(x, 6))
...
"""
@inline function rrelu(x::RealOrFloatType, l::Real = 1 / 8.0, u::Real = 1 / 3.0)
...
"""
@inline elu(x::RealOrFloatType, α = one(x)) = vifelse(x ≥ 0, x / one(x), α * (exp(x) - one(x)))
...
activation function.
"""
@inline function gelu(x::RealOrFloatType)
    p = oftype(x / 1, π)
    λ = oftype(x / 1, √(2 / p))
...
"""
@inline swish(x::RealOrFloatType) = x * σ(x)
...
"""
@inline lisht(x::RealOrFloatType) = x * tanh(x)
...
"""
@inline function selu(x::RealOrFloatType)
...
"""
@inline celu(x::RealOrFloatType, α::Real = one(x)) = vifelse(x ≥ 0, x / one(x), α * (exp(x/α) - one(x)))
...
"""
@inline trelu(x::RealOrFloatType,theta = one(x)) = vifelse(x> theta, x, zero(x))
...
"""
@inline softsign(x::RealOrFloatType) = x / (one(x) + abs(x))
...
"""
@inline softplus(x::RealOrFloatType) = vifelse(x > 0, x + log1p(exp(-x)), log1p(exp(x)))
...
"""
@inline logcosh(x::RealOrFloatType) = x + softplus(-2x) - log(oftype(x, 2))
...
"""
@inline mish(x::RealOrFloatType) = x * tanh(softplus(x))
...
"""
@inline tanhshrink(x::RealOrFloatType) = x - tanh(x)
...
"""
@inline softshrink(x::RealOrFloatType, λ = oftype(x/1, 0.5)) = min(max(zero(x), x - λ), x + λ)
