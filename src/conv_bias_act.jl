...
function conv_bias_act(
    ...
    cdims::C, b::AbstractArray{bT,N}, σ=identity; kwargs...,
) where {xT, wT, bT, N, C <: Union{ConvDims, DenseConvDims}}
...
function conv_bias_act!(
    y::AbstractArray{yT,5}, x::AbstractArray{xT,5}, w::AbstractArray{wT,5},
    cdims::C, b::AbstractArray{bT,5}, σ=identity; kwargs...,
) where {yT, xT, wT, bT, C <: Union{ConvDims, DenseConvDims}}
    conv!(y, x, w, cdims)
    y .= σ.(y .+ b)
    return y
end
